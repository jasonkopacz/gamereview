...
const format = 'json';
const guid = '3030-1';
let field_list = 'image.icon_url, image.medium_url, image.screen_url, image.screen_large_url,' + 
'image.small_url, image.super_url, image.thumb_url, image.tiny_url, image.original_url,';
let results = [];
const fetchData = async () => {
  try {
    console.log(number)
    const response = await fetch(`${baseUrl}${endpoint}/${guid}?api_key=${apiKey}&format=${format}&field_list=${field_list}`);
