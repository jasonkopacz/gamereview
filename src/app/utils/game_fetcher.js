...
const number = 1;
let field_list =
  "rating,description,developers,genres,number_of_reviews,original_release_date";
let results = [];
const fetchData = async () => {
  try {
    console.log(number);
    const response = await fetch(
      `${baseUrl}${endpoint}/${api_detail_url}?api_key=${apiKey}&format=${format}&field_list=${field_list}`
    );
...
const createAndWriteToCsv = (data) => {
  ...
  data.forEach((item) => {
    ...
    writeStream.write(`${api_detail_url},${deck},${guid},${id},${name}\n`);
    // writeStream.write(
